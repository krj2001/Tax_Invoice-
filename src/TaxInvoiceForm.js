...
//     // Calculate subtotal
//     const subtotal = dynamicInputs.reduce(
//       (acc, input) => acc + parseFloat(input.amount || 0),
//        0
//     );
...
//         navigate(`/view/${id}?subtotal=${subtotal}`);
//       } else {
//         console.error("Failed to save invoice");
//       }
//     } catch (error) {
...
const TaxInvoiceForm = () => {
  ...
  const formatNumber = (value) => {
    ...
    const formattedValue = Number(cleanedValue).toLocaleString("en");
  ...
  const handleDynamicInputChange = (index, e) => {
    ...
    if (name === "unitCost" || name === "amount") {
      formattedValue = formatNumber(value);
    }

    const newDynamicInputs = [...dynamicInputs];
    newDynamicInputs[index][name] = formattedValue;
    setDynamicInputs(newDynamicInputs);
  };
  ...
  const handleSubmit = async () => {
    ...
    const subtotal = dynamicInputs.reduce(
      (acc, input) => acc + parseFloat(input.amount.replace(/,/g, "") || 0),
      0
    );

    const formattedSubtotal = subtotal.toLocaleString("en");
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate(`/view/${id}?subtotal=${formattedSubtotal}`);
      } else {
        console.error("Failed to save invoice");
      }
    } catch (error) {
